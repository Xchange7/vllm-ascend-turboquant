...
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-higgs-smoke}"
PORT="${PORT:-8000}"
HOST="${HOST:-0.0.0.0}"
TP_SIZE="${TP_SIZE:-1}"
...
KV_CACHE_DTYPE="${KV_CACHE_DTYPE:-${HIGGS_PRESET:-turboquant_4bit_nc}}"
...
case "${KV_CACHE_DTYPE}" in
  turboquant_4bit_nc|turboquant_k3v4_nc|turboquant_3bit_nc)
    ;;
  turboquant_k8v4)
    echo "turboquant_k8v4 uses FP8 keys, not the HIGGS key quantizer."
    echo "Use turboquant_4bit_nc, turboquant_k3v4_nc, or turboquant_3bit_nc."
    exit 2
    ;;
  *)
    echo "Unsupported HIGGS preset: ${KV_CACHE_DTYPE}"
    echo "Valid presets: turboquant_4bit_nc, turboquant_k3v4_nc, turboquant_3bit_nc"
    exit 2
    ;;
esac
...
echo "HIGGS-backed KV cache dtype: ${KV_CACHE_DTYPE}"
